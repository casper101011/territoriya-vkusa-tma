  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/send-feedback') {
      ...
      try {
        ...
        if (!env.DB) return json({ ok: false, error: 'D1 database не подключена к Worker.' }, 500, corsHeaders);
        let userName = 'Не определено', userUsername = '', telegramId = '';
        ...
        if (initData) {
          ...
          if (userRaw) try {
            ...
            if (telegramId) await env.DB.prepare(`INSERT INTO users (telegram_id, username, first_name, last_name, updated_at) VALUES (?, ?, ?, ?, datetime('now')) ON CONFLICT(telegram_id) DO UPDATE SET username=excluded.username, first_name=excluded.first_name, last_name=excluded.last_name, updated_at=datetime('now')`).bind(telegramId, user.username || null, user.first_name || null, user.last_name || null).run();
          } catch (_) {}
        }
        ...
        const telegramResponse = await fetch(`https://api.telegram.org/bot${env.BOT_TOKEN}/sendMessage`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chat_id: env.CHAT_ID, text: lines.join('\n'), parse_mode: 'HTML' }) });
      ...
      } catch (error) { console.error('Feedback error:', error?.stack || error); return json({ ok: false, error: error?.message || 'Ошибка сервера.' }, 500, corsHeaders); }
    }
    const assetResponse = await env.ASSETS.fetch(request);
    const contentType = assetResponse.headers.get('content-type') || '';
    if (!contentType.includes('text/html')) return assetResponse;
    const html = await assetResponse.text();
    const feedbackOverride = `<script data-feedback-handler="v6">
...
(function setupContactRequest() {
  ...
  document.getElementById('share-phone-btn').addEventListener('click', () => tg.requestContact(shared => { if (shared) { const b = document.getElementById('share-phone-btn'); b.textContent = '✓ Номер передан Telegram'; b.disabled = true; } }));
    ...
    const headers = new Headers(assetResponse.headers); headers.delete('content-length'); headers.set('Cache-Control', 'no-store, no-cache, must-revalidate'); headers.set('X-Feedback-Handler', 'v6');
